...
def matrix():
    ...
    for i in range(n):
        a=[]        #mini arrey that we have to apend to the arrey matrix to have a complete matrix
        for j in range(m):
...
def addmatrix(m1, m2):
    m3=[]           # new arrey to store the addition of the element
    n=len(m1)
    m=len(m1[0])
    for i in range(n):
        a=[]            # mini arrey tor the rows 
        for j in range(m):
            s = m1[i][j]+m2[i][j]
            a.append(s)
        m3.append(a)
